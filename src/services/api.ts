...
export async function generateBaseImage(prompt: string, vertical: 'post' | 'reels' = 'post'): Promise<string> {
  ...
  const fullPrompt = `${prompt}. Fotografia editorial profissional, luz natural abundante e clara, cena bem iluminada, composição limpa e moderna, sem texto, sem logotipo, sem distorções anatômicas, alta qualidade.`;
